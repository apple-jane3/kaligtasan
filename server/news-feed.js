...
async function fetchRssFeed({ url, source, region }, query) {
...
async function fetchGoogleNewsArticles(query, timeoutMs = 8000) {
  const rssUrl = `https://news.google.com/rss/search?q=${encodeURIComponent(query)}&hl=en-PH&gl=PH&ceid=PH:en`;
  const xml = await fetchText(rssUrl, timeoutMs);
...
async function fetchAllSources(query) {
  const tasks = [
    ...RSS_FEEDS.map((feed) =>
      fetchRssFeed(feed, query).catch(() => [])
    ),
    ...GOOGLE_NEWS_QUERIES(query).map((searchQuery) =>
      fetchGoogleNewsArticles(searchQuery).catch(() => [])
    ),
    fetchGdeltArticles(query).catch(() => []),
  ];
...
async function fetchEarthquakeNewsArticles(query = DEFAULT_QUERY) {
  ...
  let articles = await fetchAllSources(query);
